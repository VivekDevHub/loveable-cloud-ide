...
export async function launchProject(projectId: string, userId: string) {
    ...
    try {
        try {
            ...
            console.warn("[k8s] Local mode fallback: Pod/Service creation skipped:", k8sErr?.message || k8sErr)
            project.previewUrl = `http://localhost:3002`
        }
        await recordActivity(runtimeId).catch(() => {})
